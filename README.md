...
- Qlib深度学习挑战者与候选者使用100%风险预算，不再固定40%仓位；
- 因子基线采用全候选池统一横截面Top-K和100%总风险预算；行业只做展示归因，不参与名额与权重；
- 因子、GRU挑战者和候选者共享全局Top-10排名递减组合构建器，单股目标5%～15%；
- T日信号、T+1开盘模拟撮合；
- 订单、成交、持仓、现金和对账账本；
- 交易连续性补偿规划器：故障恢复后基于最新目标与券商快照计算差额，含未完成订单门禁和持久化幂等预约；当前仅规划/验证，不向券商发送真实委托；
- 每日18:30服务端自动流水线；
- 组合研究与运行观测前台。
