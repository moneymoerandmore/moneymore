...
def main() -> int:
    ...
    try:
        with args.log_path.open("a", encoding="utf-8") as log:
            log.write(f"\n[{started_at}] weekly FinRL exposure training started\n")
            result = subprocess.run(
                [sys.executable, str(ROOT / "scripts" / "train_finrl_exposure.py"),
                 "--tag", tag, "--timesteps", "20000"],
                cwd=ROOT, env={**os.environ, "PYTHONUTF8": "1"},
                stdout=log, stderr=subprocess.STDOUT, text=True, check=False,
            )
            exit_code = result.returncode
        artifact = ROOT / "state" / "exposure-league" / "finrl" / "candidates" / tag
        metadata_path = artifact / "metadata.json"
        model_path = artifact / "model.zip"
        if exit_code == 0 and metadata_path.exists() and model_path.exists():
            metadata = json.loads(metadata_path.read_text(encoding="utf-8"))
            if metadata.get("status") != "COMPLETED" or metadata.get("timesteps") != 20000:
                error = "FinRL artifact validation failed"
                exit_code = 1
        elif exit_code == 0:
            error = "FinRL model or metadata artifact is missing"
            exit_code = 1
        if exit_code and error is None:
            error = f"FinRL trainer exited with code {exit_code}"
